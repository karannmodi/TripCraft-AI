...
| ID | Area | Test Case | Expected Result | Actual Result | Status | Verification Method | Bug ID / Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `RES-001` | Reservations | Reservation Listing & Category Filtering | Renders reservation cards with type/status badges; category pill filters (`All`, `Lodging`, `Transportation`, `Restaurant`, `Activity`) filter visible cards. | `GET /api/v1/trips/{id}/reservations` returns `200 OK` with 3 confirmed reservations (`Chicago Hotel`, `Architecture River Cruise`, `Dinner Reservation`). Category pill filtering verified in `ReservationView.tsx`. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Reservation listing and category filtering verified. |
...
| ID | Area | Test Case | Expected Result | Actual Result | Status | Verification Method | Bug ID / Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `BUD-001` | Budget | Budget Stat Cards & Category Breakdown | Accurately calculates and displays Trip Budget, Total Est Spent, Total Act Spent, Est Remaining, Act Remaining, and Category Breakdown totals using Python `Decimal`. | `GET /api/v1/trips/{id}/budget` returned `200 OK`. <br/>**Independent Math Verification**: Decimal-based calculations were independently verified for the tested Chicago Weekend values: <br/>• Trip Budget: `$1,800.00` <br/>• Total Est Spent: `$1,450.00` <br/>• Total Act Spent: `$1,425.00` <br/>• Est Remaining: `$350.00` (`1800 - 1450 = 350`) <br/>• Act Remaining: `$375.00` (`1800 - 1425 = 375`). | PASS | HYBRID (`AI-API/DB` + `AI-AUTOMATED` + `AI-CODE-INSPECTION`) | Decimal-based calculations independently verified for tested Chicago values. |
| `BUD-002` | Budget | Expense Creation & Paid Toggle | Submitting valid expense across categories (`Lodging`, `Food`, etc.) inserts DB record and updates stat cards; toggling "Is Paid" updates DB status. | `POST /api/v1/trips/{id}/expenses` returned `201 Created` for temp expense (*"Temp Souvenir T-Shirts"*). `PUT` toggled `is_paid` to `True` (`200 OK`). | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Expense creation and paid toggle verified. Temp record removed. |
...
| ID | Area | Test Case | Expected Result | Actual Result | Status | Verification Method | Bug ID / Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `PACK-001` | Packing | AI Packing Generation & Progress Display | Generates AI packing list (`gemma3:1b`) across 6 categories; checking items updates progress bar (`X/Y packed`) and percentage. | `GET /api/v1/trips/{id}/packing` returned `200 OK` with 10 items across 6 categories (6 AI-suggested, 4 custom), 7/10 packed (70.0%). `PackingView.tsx` calculates progress bar and renders category cards. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Packing item listing, category grouping, and progress calculation verified. |
| `PACK-002` | Packing | Manual Item CRUD & Checkbox Persistence | Adding custom item (`is_ai_suggested=False`) displays "Custom" badge; editing/deleting updates DB; checking item persists after page refresh. | `POST /api/v1/trips/{id}/packing/items` created custom item (`201 Created`). Toggling `is_packed=True` returned `200 OK`. Re-fetch verified persistence in PostgreSQL. Temp item deleted. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Custom packing item CRUD and persistent checkbox toggling verified. |
| `PACK-003` | Packing | Safe Regeneration & Custom Item Preservation | Regenerating AI packing list after confirmation replaces AI-suggested items while preserving manually added custom items and their packed state intact. | Created temp custom item (`is_ai_suggested=False`, `is_packed=True`). Executed live Ollama `gemma3:1b` regeneration (`POST /packing/generate?overwrite=true`). AI items refreshed while custom item ID, name, and checked state remained 100% intact! Temp item cleaned up. | PASS | HYBRID (`AI-API/DB` + `AI-OLLAMA` + `AI-CODE-INSPECTION`) | Safe AI regeneration and custom user item preservation verified with live Ollama `gemma3:1b`. |
| `PACK-004` | Packing | Packing Offline Error Handling | Offline Ollama service displays clear HTTP 503 error alert without losing existing packing items. | Key normalization handling schema variations (`item_name`, `name`, `item`) verified in `OllamaAIService._parse_and_validate_packing`. Backend returns HTTP 503 on Ollama offline state without corrupting DB. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Pydantic key normalization and offline failover verified in backend service. |
...
| ID | Area | Test Case | Expected Result | Actual Result | Status | Verification Method | Bug ID / Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `CHAT-001` | Ask My Trip | Deterministic Fact-First Q&A | Clicking suggestion pills or asking factual questions (reservations, budget spent/remaining, busiest day, packing stats) returns instant deterministic responses (< 0.1s) matching DB values. | Executed queries for reservations, budget spent ($1,425.00), budget remaining ($375.00), busiest day (Day 1). Fact-first deterministic responses are generally faster than local LLM narrative responses (measured times: 0.038s–0.085s recorded as diagnostic info). | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Fact-first Q&A verified against PostgreSQL database facts. |
| `CHAT-002` | Ask My Trip | Ollama Narrative Trip Summary | Asking "Summarize my trip" calls local Ollama model (`gemma3:1b`) with pre-calculated DB facts and renders natural narrative response. | Executed *"Summarize my trip"* via live Ollama `gemma3:1b` (diagnostic response time: 10.71s). Narrative response correctly incorporates Chicago facts ($1800 budget, $1425 spent, $375 remaining, 7/10 packing) without introducing unsupported dates/costs. | PASS | HYBRID (`AI-API/DB` + `AI-OLLAMA` + `AI-CODE-INSPECTION`) | Live local Ollama `gemma3:1b` narrative synthesis verified using exact DB facts. |
| `CHAT-003` | Ask My Trip | Unavailable Information & Empty Input | Asking for non-existent database facts returns clear refusal stating data is unavailable rather than hallucinating; submitting blank message is rejected. | Asked *"Where is my passport scan?"*. Assistant returned: *"It is unavailable in the provided trip context. The trip details include travel dates and budget but do not contain information about passport scans..."* (No hallucinated facts). | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Refusal to invent missing database facts verified. |
| `CHAT-004` | Ask My Trip | Chat History Persistence & Offline Handling | Past Q&A message thread persists across page refresh; offline Ollama service displays HTTP 503 error alert for narrative requests. | Submitting blank prompt returns `422 Unprocessable Entity`. Chat message thread persists in PostgreSQL table `chat_messages`. Offline Ollama request returns `503 Service Unavailable`. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Blank prompt rejection, chat thread persistence, and offline failover verified. |
...
| ID | Area | Test Case | Expected Result | Actual Result | Status | Verification Method | Bug ID / Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `RAI-001` | Responsible AI | Fact Grounding & Math Calculation Isolation | All financial metrics, budget remaining totals, and reservation figures are computed in Python/PostgreSQL rather than LLM inference, preventing math errors. | Source inspection (`chat_service.py`, `budget_service.py`) and runtime evidence confirm financial metrics and reservation facts are computed in Python/PostgreSQL. Decimal-based calculations were independently verified for the tested Chicago Weekend values. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Decimal-based calculations independently verified for tested Chicago values. |
| `RAI-002` | Responsible AI | AI Output Validation & Key Normalization | Pydantic schema validation and key normalization handle small LLM JSON key variations (`item_name`, `name`, `item`) with 1 controlled retry on malformed structures. | `OllamaAIService` implements Pydantic schema boundary validation (`AIPackingList`, `AIPackingItem`), key normalization, and 1 controlled retry loop for malformed JSON structures. Malformed output is rejected before DB commit. | PASS | HYBRID (`AI-API/DB` + `AI-CODE-INSPECTION`) | Pydantic boundary validation and key normalization verified. |
