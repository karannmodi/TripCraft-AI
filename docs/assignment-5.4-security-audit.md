...
| `SEC-002` | DB Connection & Privilege Separation | Verify database connections use dedicated least-privilege roles rather than PostgreSQL administrative superuser. | Initial audit identified `config.py` fallback using `postgres` superuser. Intermediate step changed fallback to `tripcraft_app`. Final remediation removed default fallback string entirely, requiring `DATABASE_URL` from environment configuration. | HARDENING | **SEC-FINDING-001 (Severity: Low)**: Initial default fallback referenced administrative `postgres` superuser; intermediate step retained hardcoded role credentials in source code. | **RESOLVED** |
...
### SEC-FINDING-001: Removal of Hardcoded Database Fallback Credentials
...
- **First Remediation Attempt**: Replaced the administrative `postgres` superuser default fallback string in `config.py` with a dedicated application role fallback (`postgresql+asyncpg://tripcraft_app:tripcraft_pass@localhost:5432/tripcraft`).
- **Review Finding**: Security review identified that the revised fallback still contained hardcoded database credentials (`tripcraft_pass`) in committed source code.
- **Final Remediation**: Removed the credential-bearing fallback string entirely from `backend/app/core/config.py`. Made `DATABASE_URL: str` an environment-required setting without default credentials or fallback strings. If `DATABASE_URL` is omitted, the application fails fast at startup with a Pydantic `ValidationError: Field required [type=missing]` rather than attempting silent connection with fallback credentials.
- **Final Verification Result**: 
  1. Existing local `.env` successfully loads `DATABASE_URL` for the dedicated `tripcraft_app` role at runtime.
  2. FastAPI started successfully and `GET /api/v1/health` returned `200 OK` (`status: healthy`, `database_status: connected`, `database_engine: postgresql+asyncpg`).
  3. Repository searches for `tripcraft_pass`, `postgres:postgres`, and `postgresql+asyncpg://` in code returned **zero matches**.
  4. Isolated configuration test confirmed missing `DATABASE_URL` triggers immediate fail-fast `ValidationError`.
  5. Database passwords and `.env` files remain 100% excluded from Git tracking.
...
Following the complete remediation of `SEC-FINDING-001` and `SEC-FINDING-002`, all 8 security audit checks (`SEC-001` through `SEC-008`) are verified satisfactorily with **0 open findings, 0 hardcoded credentials, 0 critical risks, and 0 unresolved issues**.
